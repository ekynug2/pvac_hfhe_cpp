...
int main() {
    std::cout << "- bounty3 gen -\n";

    
    
    //!
    const std::string seed = "mnemonic: (mnemonic phrase for octra wallet), number: (the code you will need to send a tx with a message that should be this secret number)";
    //!
    ...
    std::cout << "enc [" << seed.size() << " bytes]\n";
    auto cts = enc_text(pk, sk, seed);
    std::cout << "cts: " << cts.size() << "\n";
    ...
    std::cout << "canon: " << std::hex << pk.canon_tag << std::dec << "\n";
    std::cout << "ok\n";
    return 0;
}
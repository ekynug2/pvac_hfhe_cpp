...
// === IO Helpers (Same as bounty3_test.cpp) ===
namespace Magic {
...
int main() {
    ...
    try {
        ...
        std::cout << "[*] Loading Ciphertext (Raw Parsing for Optimization)...\n";
        // We load manually to potentially decrypt faster or handle OOM better
        std::ifstream i(dir + "/seed.ct", std::ios::binary);
        if (io::get32(i) != Magic::CT || io::get32(i) != Magic::VER) throw std::runtime_error("bad ct");
        uint64_t ncts = io::get64(i);
        
        // We need to load the full Cipher structures first as dec_text takes a vector<Cipher>
        std::vector<Cipher> cts;
        cts.reserve(ncts);
        for (size_t j = 0; j < ncts; ++j) {
            cts.push_back(ser::getCipher(i));
        }
        std::cout << "[+] Ciphertext Loaded (" << cts.size() << " blocks).\n";

        std::cout << "[*] Starting Decryption (Attempting full dec_text)...\n";
        // This is the heavy call. 
        // If this fails with bad_alloc, we cannot decrypt fully in this environment.
        
        // OPTIMIZATION TRICK: 
        // Before decrypting, reduce the edge budget? 
        // No, we can't modify the loaded pk/prm easily.
        // We must try standard decryption.
        
        std::string recovered;
        try {
            recovered = dec_text(pk, sk, cts);
            std::cout << "\n✅ SUCCESS:\n";
            std::cout << recovered << "\n";
        } catch (const std::bad_alloc& e) {
            std::cerr << "\n[-] FAILED: std::bad_alloc (Out of Memory)\n";
            std::cerr << "[!] Your system RAM is insufficient for the full decryption graph.\n";
            std::cerr << "[!] Recommended: Increase WSL memory or move to a machine with 16GB+ RAM.\n";
            return 1;
